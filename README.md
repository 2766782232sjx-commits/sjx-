...
纯静态 SPA：`index.html`（全部逻辑内联）+ `data.js`（岗位库629条）+ `announcements.js`（公告）+ `backup.json`（云端备份）。无后端、无依赖，GitHub Pages 直接托管。
...
- 2026-09-06 晚批次：行业速查卡字体统一+CATS配色改莫兰迪色系；简历画像卡新增「当前战报」（美团境内住宿策略产品意向✅/拼多多管培提前批泡池）；岗位库「竞争激烈程度」默认折叠；励志卡改为「👑 今日份 女王语录」（小红书高赞强女文案24条）；「我的投递」每条新增自由记录区（岗位JD粘贴/准备情况/面经，自动保存随备份迁移）；BBS爬8页发现并补充15条未覆盖岗位（洛阳钼业矿业出海、冬海Sea、Duolingo、法巴BNP、华兴资本、中再集团、建信信托、量化职能线、国资数据集团、央企政研岗大通道等），岗位库614→629条
